...
contract AuthorizeContract is Ownable, ERC721Holder, Suzuki {
    ...
    function _balanceOf(address _address) external view returns (uint256) {
    ...
    function buyNft(IERC20 _token) external {
        ...
        require(
            getAuthorizedUserStatus(msg.sender) == true,"Not authorized user"
        );
    ...
    function calculateTokenAmount() public view returns (uint256) {
        uint256 mintBlockNumber = getBlockNumber();
